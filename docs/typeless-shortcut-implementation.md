...
**与 Xisper 的核心差异**：
- Typeless 用 **koffi FFI + 预编译 Swift dylib**，不需要 Rust 编译步骤
- Typeless 的 Swift 库是预编译好的 `.dylib`，部署简单
- Xisper 用 **NAPI + Rust**，需要在 CI 编译 `.node` 文件
...
// BqIt5gYv.mjs - 连接 hC hook 与 XState 机
...
**关键发现**：Typeless **没有** Xisper 那样的"左侧修饰键单独禁止"规则。右侧修饰键可以作为独立快捷键（Swift 库能检测 RightCmd 等），但默认推荐使用 FN 键。
